...
async function _bugName(db, bug_id, dev, user_name) {
  const bug = await CRUDService.getBySearch(db, TABLE_NAMES.BUG, "id", bug_id);
...
commentRouter
  ...
  .get(async (req, res, next) => {
    try {
      ...
      const comments = SerializeService.formatAll(rawComments, TABLE_NAME);
...
commentRouter
  ...
  .all(async (req, res, next) => {
    try {
      ...
      const bug_name = await _bugName(req.app.get("db"), id, dev, user_name);
...
commentRouter.route("/bug/:bugId").get(async (req, res, next) => {
  try {
    ...
    const bug_name = await _bugName(req.app.get("db"), bugId, dev, user_name);
